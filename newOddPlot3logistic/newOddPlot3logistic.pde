...
logistic equation for both x and y
functions:
  f // put values in nextx and nexty
        ...
        lower r values have more detail
x and y range:
-7 to 7
  ...
  for (int j = 0; j <height; j++) {
    for (int i =0; i <width; i++) {
      ...
      while (iter < maxiter) {
        ...
        float nextx = r*(y-y*y)-r*(x-x*x);
        float nexty = (r*(-2)*x*y); 
        // swap values so x and y become nextx and nexty and are fed into function
        x = nextx+cx; // remove cx and cy you get just a sun
        y = nexty+cy; // set a single number for cx and cy you get the julia set
